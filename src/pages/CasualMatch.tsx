...
import { getSportStats } from '../lib/sports';
...
export default function CasualMatch({ search }: CasualMatchProps) {
  ...
  const handleChallenge = async (opponent: UserProfile) => {
    try {
      await createCasualMatch(opponent.uid, sport);
      window.alert(t('play.challengeSent', { name: opponent.displayName }));
    } catch (error) {
      console.error('Error creating match', error);
      window.alert(t('play.challengeFailed'));
    }
  };
      ...
      <section>
        <div className="space-y-3">
          {filteredUsers.map((currentUser) => {
            const currentStats = getSportStats(currentUser, sport);
            ...
            return (
              <div
                ...
                className={clsx(
                  ...
                  theme === 'dark'
                    ? 'border-white/5 bg-zinc-900/30 hover:bg-zinc-800/30'
              ...
              >
                <div className="flex min-w-0 items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 truncate text-xs font-medium text-zinc-500">@{currentUser.nickname}</div>
                  </div>
                </div>
                ...
                <div className="flex shrink-0 items-center gap-3">
                  <div className="text-right">
                    <div className="flex items-center justify-end gap-1.5 text-lg font-black text-emerald-500">
                      <span>{currentStats.casualStars}</span>
