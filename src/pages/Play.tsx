...
export default function Play() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      <header className="-mx-4 -mt-4 sticky top-0 z-40 pb-3 md:mx-0 md:mt-0">
        <div
          className={clsx(
        ...
        >
          <span
            className={clsx(
              ...
              theme === 'dark'
                ? 'bg-[linear-gradient(90deg,rgba(16,185,129,0.16),rgba(255,255,255,0.08),rgba(59,130,246,0.14))]'
                : 'bg-[linear-gradient(90deg,rgba(16,185,129,0.12),rgba(255,255,255,0.88),rgba(59,130,246,0.12))]',
            )}
          />
          <div className="relative">
            <div className="mb-4 flex items-end justify-between gap-4">
              <div>
                <h1 className={clsx('text-3xl font-bold tracking-tight', theme === 'dark' ? 'text-white' : 'text-zinc-900')}>
                  {t('nav.play')}
                </h1>
                <p className={clsx('mt-1 text-sm font-medium', theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500')}>
                  {getSportShortLabel(sport, language)}
                </p>
              </div>
            </div>

            <SportToggle sport={sport} onChange={setSport} theme={theme} language={language} className="mb-4" />
